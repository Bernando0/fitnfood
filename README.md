...
Бот — **жёсткий честный фитнес-тренер**: не подлизывается, прямо критикует мусорную еду и
не занижает калории. Критикует ЕДУ И ВЫБОР, а не личность/вес человека.

Каждый приём попадает в зону 🟢🟡🔴 по правилу из Noom + UK FSA + NOVA
(детали и источники — [docs/FOOD_ZONES.md](docs/FOOD_ZONES.md)). День агрегируется в одну
зону (взвешенно по калориям); день без единого фото — ⚪ **серая зона**.

Участники ведутся **по чату**: в группе — каждый отдельно, в личке — один.

## Команды

- `/help` — справка
- 📷 фото еды (можно с подписью: состав/порция) · ✍️ `/ate <что съел>` — лог текстом
- `/ask <вопрос>` — совет с учётом твоей еды и цели
- `/eat [продукты]` — что лучше съесть сейчас (можно указать, что есть дома)
- `/undo` — удалить последний приём (или кнопкой 🗑 под ответом)
- `/stats` — аналитика по дням за неделю (🟢🟡🔴⚪ + калории)
- `/tone` — выбрать тон чата: 🔥 жёстко с матом / 💪 тренер без мата / 🙂 поддерживающий
